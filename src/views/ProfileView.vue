  ...
  <div class="min-h-screen">
    ...
    <main class="container-content py-14">
      <div class="mx-auto max-w-6xl">
        <h1 class="font-serif text-h2 font-bold text-text-primary">Profile</h1>
        ...
        <div class="mt-8 grid grid-cols-1 gap-6 md:grid-cols-3">
          ...
          <section class="card-base p-6 space-y-4">
            ...
            <form novalidate class="space-y-4" @submit.prevent="submitProfile">
              <div>
                <label
                  for="profile-email"
              ...
              <div class="grid grid-cols-[auto_1fr] gap-x-2 gap-y-1">
                <div>
                  ...
                  />
                  <p
                    v-if="profileErrors.countryCode"
                    class="mt-1 text-sm text-red-400"
                    role="alert"
                  >
                ...
                <div>
                  ...
                  />
                  <p
                    v-if="profileErrors.phoneNumberNormalized"
                    class="mt-1 text-sm text-red-400"
                    role="alert"
                  >
          ...
          <section class="card-base p-6 space-y-4">
            ...
            <form novalidate class="space-y-4" @submit.prevent="submitPassword">
              <div>
                <label
                  for="pw-current"
                  class="block text-sm font-medium text-text-primary"
          ...
          <section class="card-base p-6 space-y-4 border-red-500/30">
            ...
            <div class="space-y-3">
              <p class="text-sm text-text-secondary">
                Deactivating your account will log you out on all devices. You
                will not be able to sign in until the account is reactivated.
              </p>
              <Button
            ...
            <div class="pt-4 border-t border-white/10 space-y-3">
              <p class="text-sm text-text-secondary">
                ...
                uploaded files, and all associated data. This cannot be undone.
              </p>
              <Button
    ...
    >
      <div class="card-base w-full max-w-md p-6 space-y-4" @click.stop>
        <h2
          id="delete-dialog-title"
          class="font-serif text-lg font-semibold text-red-400"
        >
          Delete account
        </h2>
        <p class="text-sm text-text-secondary">
          Enter your current password to confirm. All your data will be
          permanently deleted.
        </p>
        <div>
          <label
...
async function submitProfile() {
  ...
  if (!parsed.success) {
    ...
    if (flat.phoneNumberNormalized)
      profileErrors.phoneNumberNormalized = flat.phoneNumberNormalized[0] ?? "";
    profileError.value =
      flat.email?.[0] ??
      flat.firstName?.[0] ??
      flat.lastName?.[0] ??
      flat.countryCode?.[0] ??
      flat.phoneNumberNormalized?.[0] ??
  ...
  if (data.phoneNumberNormalized)
    payload.phone_number_normalized = data.phoneNumberNormalized;
    ...
    if (result.ok) {
      ...
      if (result.email_pending_verification && emailToVerify) {
        ...
        router.push({
          ...
          query: {
            email: emailToVerify,
            message: "Verify your new email to sign in again.",
          },
        });
        return;
      }
...
async function submitPassword() {
  ...
  if (!parsed.success) {
    const flat = parsed.error.flatten().fieldErrors;
    if (flat.currentPassword)
      passwordErrors.currentPassword = flat.currentPassword[0] ?? "";
    if (flat.newPassword)
      passwordErrors.newPassword = flat.newPassword[0] ?? "";
    if (flat.confirmPassword)
      passwordErrors.confirmPassword = flat.confirmPassword[0] ?? "";
    return;
  }
  ...
  try {
    ...
    if (result.ok) {
      ...
      router.push({
        path: "/",
        query: { message: "Password updated. Please log in again." },
      });
      return;
    }
...
function confirmDeactivate() {
  if (
    !window.confirm(
      "Deactivate your account? You will be logged out on all devices.",
    )
  )
    return;
...
async function submitDelete() {
  ...
  try {
    ...
    if (result.ok) {
      ...
      router.push({
        path: "/",
        query: { message: "Your account has been deleted." },
      });
      return;
    }
